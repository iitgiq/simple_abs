...
module SimpleAbs
  ...
  def ab_test_with_frequency(name, tests, impression)
    ...
    find_or_create_by_experiment_and_which_method(name, test_value).increment!(:participants, impression)
  ...
  def converted_with_frequency!(name, frequency)
    ...
    if !is_bot?
      test_value = cookies[name]
      if test_value
        find_or_create_by_experiment_and_which_method(name, test_value).increment!(:conversions, frequency)
      end
    end
  end
